...
public class CarPark {
	...
	protected void initialize() {
		...
		for (int i = 0; i < carList.length; i++) {
			...
			carList[i].setConsumption((2 + generator.nextInt(20) + (float)Math.rint(generator.nextDouble()*10)/10));
		}
		calculateTotalCost();
	}
	...
	public void searchCar() {
		...
		scan.close();
		for(int i=0;i<carList.length;i++){
			if(type.compareTo(carList[i].getType())==0 && Math.abs((consumption-carList[i].getConsumption()))<=0.2 && carList[i].getCost()==cost){
