...
public class TransportSystem {
	private static final Logger LOGGER = Logger.getLogger(TransportSystem.class);
	...
	public void addWays(String pathWays1) {
		...
		try {
			...
			fr.close();
			LOGGER.info("Reading from \"Ways.txt\" file is finished well!");
		} catch (IOException e) {
			System.out.println("Mistake: 		" + e.toString());
			LOGGER.error("Could not find the file Ways.txt! or wrong types in file!");//, e);
		}
	}
	 ...
	 * @return void
	 * */
	public void addTrans(String pathWays1) {
		...
		try {
			...
			fr.close();
			LOGGER.info("Reading from \"Transport.txt\" file is finished well!");
		} catch (IOException e) {
			System.out.println("Ошибка: 		" + e.toString());
			LOGGER.error("Could not find the file Transport.txt!");//, e);
		}
	 ...
	 * @return void
	 * */
	public void addBreakage(String pathWays1) {
		...
		try {
			...
			fr.close();
			LOGGER.info("Reading from \"Breakage.txt\" file is finished well!");
		} catch (IOException e) {
			System.out.println("Ошибка: 		" + e.toString());
			LOGGER.error("Could not find the file Breakage.txt!");//, e);
		}
	}
	...
	public void makeAssignment(String pathWays1) {
		...
		u = 1;
		k = 1;
		while (k <= size) {//k-assignment count, u - transport count																								//убрать равно!!!!!!!!!!!!!!
			...
			if (u+1 > this.trans.size()) {																															//убрать +1 !!!!!!!!! //	System.out.println("Не хватило!");	//	k = size;	
		...
		System.out.println("how much transports have been assigned for a day:	buses "
				+ autoNumber + " 	trolleybuses " + tralNumber + " 	trams "
				+ tramNumber);
		System.out.println();
		...
		try {
			if (outputFile.createNewFile())
				System.out.println("File " + outputFile.getName() + " was created.");
			else {
				outputFile.delete();
			...
			for (u = 0; u < this.assignment.size(); u++) {
				String str = "";
				if (this.trans.get(this.assignment.get(u).getIdTrans()-1)
						.getTransType() == 0)
					str = "автобус	";
				if (this.trans.get(this.assignment.get(u).getIdTrans()-1)
						.getTransType() == 1)
			...
			pw.close();
			LOGGER.info("Writing into an \"Assignment.txt\" file is finished well!");
		} catch (IOException e) {
			System.err.println(e);
			LOGGER.error("Problems with writing into an \"Assignment.txt\" file...");
		}
	...
	public void verification(){//(String pathWays1) {
		...
		LOGGER.debug("how much transports do we have  in park: 	buses "
				+ autoN + " 	trolleybuses " + tralN + " 	trams " + tramN);
		LOGGER.debug("how much transports do we need for a day: buses " + autoNumber
				+ " 	trolleybuses " + tralNumber + " 	trams " + tramNumber);
		LOGGER.debug("how much transports is broken: 		buses " + autoBrN
				+ " 	trolleybuses " + tralBrN + " 	trams " + tramBrN);
		...
		LOGGER.debug("how much is not enough: 			buses " + fauto
				+ " 	trolleybuses " + ftral + " 		trams " + ftram+"\n");
		//System.out.println();
		...
		/*// this code creates file with additional info about the transports way...
		 ...
		 */
		LOGGER.info("Verification is finished!");
	}
	...
	private static void unpack(String destinationDirectory) {
		...
		try {
			...
			LOGGER.error("Problems with unpacking transport.zip!");
			ioe.printStackTrace();
		}
		LOGGER.info("Unpacking is finished!");
	}
	/*
	...
	private static void delete(String pathWays) {
		...
		fileBrFile.delete();
		LOGGER.info("Deleting files is finished!");
	}
	...
	private static String readPath() {
		...
		try {
			...
			LOGGER.error("Could not find the file message.properties!!", e);
			e.printStackTrace();
			return null;
		}
		String message = props.getProperty("path");
		LOGGER.info("Reading from \"message.properties\" is finished well! The path is " + message);
		return message;
	}
